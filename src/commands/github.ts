...
export default function (message: Discord.Message) {
	if (message.content.match(/^-gh/i)) argument = message.content.slice(4)
	...
	if (argument.match(/^rate-limit$/i)) {
		...
		if (!argument.match(/^user[\s]+[A-Za-z0-9\-]+$/)) {
			message.channel.send(
				'No username provided or more than 1 username provided. BotRanger can only send GitHub user data once at a time'
			)
			return
		}
		...
		if (!argument.match(/^org[\s]+[A-Za-z0-9\-]+$/)) {
			message.channel.send(
				'No username provided or more than 1 username provided. BotRanger can handle only 1 GitHub user at a time'
			)
			return
		}
		fetch(url + '/orgs/' + argument.match(/[A-Za-z1-9\-]+$/))
			.then((response) => {
				if (response.status !== 200) {
					message.channel.send(
						'GitHub organization not found. If you are searching for a user try running `-gh user <username>`'
					)
					return
				}
				response.json().then(function (data) {
		...
		fetch(url + '/repos/' + argument.match(/[A-Za-z1-9\-]+[\/][A-Za-z1-9\-]+$/))
			.then((response) => {
				if (response.status !== 200) {
					message.channel.send(
						'GitHub repo not found. For more help with the GitHub command run `-help github`'
					)
					return
				}
				response.json().then(function (data) {
					let msg = new Discord.MessageEmbed()
						...
						.addFields(
							{
								name: 'License',
								value: data?.license?.spdx_id ?? 'No License',
								inline: true,
							},
		...
		message.channel.send(
			'No overload matched this query. Try running `-help gh` to get help about the github command'
		)
	}
}
